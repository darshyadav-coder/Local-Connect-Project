...
if (navigator.geolocation && locationText) {                     //check if browser supports geolocation AND element exists
    navigator.geolocation.getCurrentPosition (                   // gets user's current position (lat and long)
       async (position) => {                                     // success function (runs if user allows location access)
            ...
            try {                                               // send request to openstreetmap api to convert lat/lon into place name
