...
const createBooking = async (req, res) => {
  try {
    ...
    const booking = new Booking({
      ...
      paymentTiming: paymentTiming || 'After Service',
...
const addFeedback = async (req, res) => {
  try {
    ...
    // Create a record in the Feedback collection for the "Recent Reviews" section
    try {
      // Extract numeric rating from string like "⭐⭐⭐⭐⭐ 5"
      const ratingStr = feedback.rating ? feedback.rating.toString() : "5";
      const ratingMatch = ratingStr.match(/\d+/);
      const ratingValue = ratingMatch ? parseInt(ratingMatch[0]) : 5;

      const newFeedback = new Feedback({
        booking: booking._id,
        userEmail: booking.userEmail,
        userName: booking.userName || booking.customerName || 'Anonymous',
        service: booking.service,
        rating: ratingValue,
        comment: feedback.comment || '',
        providerId: booking.provider || null
      });
      await newFeedback.save();
    } catch (feedErr) {
      console.error('Failed to create secondary feedback record:', feedErr.message);
      // We don't return error here because the primary update succeeded
    }
